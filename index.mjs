...
class Logger {
    ...
    constructor(local,options) {
        ...
        this.error = (text) => {
            ...
            if(this.local)fs.appendFileSync(path.resolve(this.local,this.time().split(' ')[0] + '.' + this.format),'[ERR] '+this.time() + ' # ' + text+'\n')
        }
        this.info = (text) => {
            console.log(coloral.markblue('[INF]') + ' ' + this.time() + ' # ' + text)
            if(this.local)fs.appendFileSync(path.resolve(this.local,this.time().split(' ')[0] + '.' + this.format),'[INF] '+this.time() + ' # ' + text+'\n')
        }
        this.warn = (text) => {
            console.log(coloral.markyellow('[WRN]') + ' ' + this.time() + ' # ' + text)
            if(this.local)fs.appendFileSync(path.resolve(this.local,this.time().split(' ')[0] + '.' + this.format),'[WRN] '+this.time() + ' # ' + text+'\n')
        }
        this.done = (text) => {
            console.log(coloral.markgreen('[DON]') + ' ' + this.time() + ' # ' + text)
            if(this.local)fs.appendFileSync(path.resolve(this.local,this.time().split(' ')[0] + '.' + this.format),'[DON] '+this.time() + ' # ' + text+'\n')
        }
    }
}
